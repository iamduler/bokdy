...
| Wave | Module | Flow IDs | HTTP today | Wave status |
| --- | --- | --- | --- | --- |
| W1 | identity | F-AUTH | auth + me mounted; outbox/audit platform | partial |
...
| Wave | Module | Flow IDs | Wave status |
| --- | --- | --- | --- |
...
| done | none yet vs full UC | — |

## Next implement (after checklists accepted)

Next: finish W1 `X-Client` gates + `UC-AUTH-007`, then W2 branch + remaining staff. Do not open W3 until W2 branch create works.

FE wiring: none until the consumed row is `done` and OpenAPI lists the operation.
